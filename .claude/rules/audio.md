...
- Classifier le type **uniquement à l'insertion** via SoLoud (`getLength`) sur le fichier local ; sans fichier téléchargé → `typeDetected: false` (non classé) ; le type en base ne change jamais sauf via `updateSoundType`
